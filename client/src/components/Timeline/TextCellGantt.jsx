...
const TextCellGantt = ({ taskID, name, isDuration, updateCell}) => {
  ...
  const handleDoubleClick = () => {
    // Kích hoạt chế độ chỉnh sửa khi người dùng nhấp đúp chuột vào ô văn bản
    setIsEditing(true);
  };
      ...
      {isEditing ? (
        <div className="relative">
          <input
            ...
            className="px-4 py-2 border-b-2 border-blue-500 rounded-lg focus:outline-none"
          />
          <span className="absolute inset-y-0 right-0 flex items-center pr-3">
            <button onClick={handleBlur} className="text-blue-500 hover:text-blue-700">Done</button>
          </span>
        </div>
      ) : (
